<template>
  <v-container class="px-0 py-0">
    ...
    <gas-controller-panel :estimate="estimate" ref="gasCtx" />
    <v-row justify="center" class="px-0">
      <v-col v-if="validNeedApprove" cols="5">
        <v-btn block @click.stop="approveAndChargeHandler" color="primary" small>
          {{ $t('l.approve') }}
        </v-btn>
      </v-col>
      <v-col :cols="validNeedApprove ? 5 : 10">
        <v-btn @click.stop="rechargeHandler" block color="primary" small>
          {{ $t('l.recharge') }}
        </v-btn>
      </v-col>
    </v-row>
...
export default {
  ...
  computed: {
    ...
    estimate() {
      let es = 21000;
      if (this.validNeedApprove) {
        es = this.estimateApproved;
      } else {
        es = this.estimateRecharged;
      }
      return es;
    },
  },
      ...
      try {
        ...
        web3js.eth
          ...
          .then(async (receipt) => {
            ...
            whisperer
              .sendSimpleMessage(API_RT_RELOAD_CHAIN_BALANCES, { from: 'Approved Success' })
      ...
      try {
        ...
        web3js.eth
          ...
          .then(async (receipt) => {
            ...
            logger.debug('>>>>>', await _retTxState);
            whisperer
              .sendSimpleMessage(API_RT_ADDORUP_TX_STATE, await _retTxState)
              .then((respState) => {
                respState &&
                  respState.chainTxs &&
                  that.$store.dispatch('web3/updateChainTxs', respState.chainTxs);
