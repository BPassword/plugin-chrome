...
async function setupController(initState) {
  ...
  /**
   * when extension runtime starting initialize.
   */
    ...
    if (env3 && LOG_LEVEL === 'DEBUG') {
      setTimeout(async () => {
        try {
          ...
          const prikey = await controller.accountController.getSubPriKey();
          await controller.websiteController.unlock(prikey);
          await controller.mobileController.unlock(prikey);
        } catch (error) {
